...
int main()
{
    ...
    while (!halt)
    {
        clk++;
        reg[x0].val = 0;
        reg[x0].prod = -1; // period initialize
        if (flush_val)
        { // flushing
        ...
        // update
        decoder.id = fetcher_output.first;
        decoder.opcode = fetcher_output.second;
        issuer.id = decode_output.first;
        issuer.cur_op = decode_output.second;
        pc.val = new_pc_val;
        broadcast(alu_cdb);
        broadcast(lsu_cdb);
        broadcast(bru_cdb);
        broadcast(pre_alu_cdb);
        broadcast(pre_lsu_cdb);
        broadcast(pre_bru_cdb);
        pre_alu_cdb = alu_cdb;
        pre_lsu_cdb = lsu_cdb;
        pre_bru_cdb = bru_cdb;
        alu.update();
